...
if ~exist('v', "var")
  fprintf("> ERROR: Please input a proper vector (null).\n");
  return;
end

% Check if it's empty or a scalar
if length(v) < 2 || isempty(v)
  fprintf("> ERROR: Please input a proper vector (scalar/empty).\n");
  ...
  [row, ~] = size(v);
  if row > 1
    fprintf("> ERROR: Please input a proper vector (matrix).\n");
    return;
  end
end
